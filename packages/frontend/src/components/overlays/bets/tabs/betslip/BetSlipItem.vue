<template>
  <div class="m-2 px-3 py-3 bg-gray-100 border border-gray-200 flex flex-col space-y-3 rounded">
    <div class="flex flex-col space-y-1 text-gray-800">
      <span class="text-sm font-bold">{{ betslip.event.getName() }}</span>
      <span class="text-xs font-semibold text-gray-800" v-if="betslip.selection == selections.HOME">{{ betslip.event.get("home").get("name") }}</span>
      <span class="text-xs font-semibold text-gray-800" v-if="betslip.selection == selections.AWAY">{{ betslip.event.get("away").get("name") }}</span>
    ...
    <div class="flex flex-row w-full space-x-3">
      ...
      <button
        ...
        class="inline-flex w-full justify-center py-2 px-4 border border-gray-600 shadow-sm text-sm font-medium rounded text-gray-700 hover:text-white bg-transparent hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 transition-colors"
      >
        Remove
      </button>
    </div>
