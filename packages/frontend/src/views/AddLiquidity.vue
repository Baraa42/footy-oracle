  ...
  <div class="p-4" v-if="isAuthenticated">
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="w-full rounded shadow-sm lg:max-w-screen-xl m-auto bg-gray-800 p-6">
        ...
        <span class="text-2xl font-medium text-gray-900 bg-gray-100">{{ getTotalLPDeposit(nft) }}</span>
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  async setup() {
    ...
    const { isAuthenticated, nfts, depositNfts } = await useMoralis();
    ...
    const depositLiquidity = async (price: any) => {
      ...
      */
      ...
      marketMakerContract.methods.deposit("uri3")
