...
contract Lossless is Ownable {
    ...
    mapping(uint256 => address) homeBetPlacer;
    mapping(uint256 => address) awayBetPlacer;
    mapping(address => uint256) playerBalance;
    mapping(address => uint256) playerHomePoints;
    mapping(address => uint256) playerAwayPoints;
    enum MatchStatus {
        OPEN,
        RUNNING,
        FINISHED,
