...
contract BettingAIO is Ownable, ChainlinkClient {
    ...
    function createBackBet(
        ...
        public
        payable
        checkGameRunning(_objectId)
        amountGreaterZero(msg.value)
        checkOdds(_odds)
    {
        ...
        checkOdds(_odds)
    {
        ...
        require(
            liqidity == msg.value,
            "Liqidity and send value are not equal!"
        );
        placeBet(
            _objectId,
            BetSide.Lay,
    ...
    function placeBet(
        ...
        BetSide oppositeSide = (BetSide.Back == _betSide)
            ? BetSide.Lay
            : BetSide.Back;
        UnmatchedBetAmount[] memory unmatchedBetsArray = unmatchedBets[
            _objectId
        ][oppositeSide][_betType][_selection][_odds];
        ...
        for (uint256 i = 0; i < unmatchedBetsArray.length; i++) {
            ...
            // update unmatched bet with remaining amount if partly matched
                ...
                emit UnmatchedBetUpdated(
                    _objectId,
                    oppositeSide,
                    _betType,
                    _selection,
                    _odds,
                    unmatchedBetsArray[i].amount,
                    unmatchedBetsArray[i].from
                );
            }
    ...
    function placeMatchedBet(
        ...
        betPossibleProfit[_objectId][_betSide][_betType][_selection][_odds][
            _from
        ] += profit;

        matchedBets[_objectId][_betSide][_betType][_selection].push(
            MatchedBet(_amount, _odds, _from, false)
        );
        emit MatchedBetPlaced(
            _objectId,
            _betSide,
            _betType,
            _selection,
    ...
    function placeUnmatchedBet(
        ...
        unmatchedBets[_objectId][_betSide][_betType][_selection][_odds].push(
            UnmatchedBetAmount(_amount, _from)
        );
        emit UnmatchedBetPlaced(
            _objectId,
            _betSide,
            _betType,
            _selection,
    ...
    function withdraw(string memory _objectId) internal {
        ...
        for (uint8 i = 0; i < 1; i++) {
            ...
            for (uint8 y = 0; y < 3; y++) {
                ...
                bool hasBackSideWon = hasMatchWinnerWon(
                    _objectId,
                    BetSide.Back,
                    selection
                );
                ...
                BetSide winnerSide = (hasBackSideWon == true)
                    ? BetSide.Back
                    : BetSide.Lay;
                MatchedBet[] memory wonBets = matchedBets[_objectId][
                    winnerSide
                ][betType][selection];
                ...
                for (uint256 z = 0; z < wonBets.length; z++) {
                    ...
                    transferAmount(
                        wonBets[z].from,
                        betPossibleProfit[_objectId][winnerSide][betType][
                            selection
                        ][wonBets[z].odds][wonBets[z].from]
                    );
                    delete betPossibleProfit[_objectId][winnerSide][betType][
                        selection
                    ][wonBets[z].odds][wonBets[z].from];
                }
            }
    ...
    function transferBetToNFT(
        ...
        for (
            uint256 i = 0;
            i < matchedBets[_objectId][_betSide][_betType][_selection].length;
            i++
        ) {
            if (
                matchedBets[_objectId][_betSide][_betType][_selection][i]
                    .from ==
                msg.sender &&
                matchedBets[_objectId][_betSide][_betType][_selection][i]
                    .odds ==
        ...
        nftPossibleProfit[_objectId][tokenId] = betPossibleProfit[_objectId][
            _betSide
        ][_betType][_selection][_odds][msg.sender];
        delete betPossibleProfit[_objectId][_betSide][_betType][_selection][
            _odds
        ][msg.sender];
    ...
    function withdrawWithNFT(string calldata _objectId, uint256 tokenId)
        external
        payable
    {
     ...
     */
    function requestResult(string memory _objectId)
        public
