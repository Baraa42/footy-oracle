...
contract MockOracle is ChainlinkRequestInterface, LinkTokenReceiver {
    ...
    function getResult(string calldata _objectId)
        external
        view
        returns (uint8)
    {
        return results[_objectId];
    }

    function setResult(string calldata _objectId, uint8 winner) external {
        results[_objectId] = winner;
    }
    ...
    function oracleRequest(
        ...
        // prase object id parm from _data bytes
        string memory objectId = string(bytes(_data[11:]));
        // not the right way, fulfillOracleRequest should be called externaly
        fulfillOracleRequest(requestId, getResultAsBytes32(objectId));
    }
    ...
    function getResultAsBytes32(string memory _objectId)
        internal
        returns (bytes32)
    {
        uint8 result = results[_objectId];
        uint256 dataUint = (stringToUint(_objectId) * 10) + result;
        string memory dataString = uint2str(dataUint);
        return stringToBytes32(dataString);
    }
